...
impl nonebot_rs::prelude::ScheduledJob for TempFileClean{
    ...
    fn call(&self, _: std::sync::Arc<nonebot_rs::prelude::Bot>) -> std::pin::Pin<Box<dyn std::future::Future<Output=()> + Send + 'static>> {
        Box::pin(async move{
            ...
            match std::fs::remove_dir_all(&path) {
                Ok(_) => {
                    ...
                    nonebot_rs::prelude::log_event!(nonebot_rs::prelude::Level::INFO,"临时文件删除成功!");
                }
                Err(err) => {
                    nonebot_rs::prelude::log_event!(nonebot_rs::prelude::Level::INFO,"临时文件删除失败! 错误:{}",err);
                }
            }
        })
    }
